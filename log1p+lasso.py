...
def prepare_data(train_years, test_years):
    ...
    train_tf = vectorizer.fit_transform(train_texts)  # 只在训练集fit
    test_tf = vectorizer.transform(test_texts)        # 测试集用相同vectorizer
    train_tf=np.log1p(train_tf)
    test_tf=np.log1p(test_tf)
    ...
    X_train = merge_features(train_tf, train_logvol_minus)
    X_test = merge_features(test_tf, test_logvol_minus)
